...
class MongoDBCollectionStatSender(object):
    def __init__(self,
                 dbName,
                 server,
                 mongoClient = None,
                 ...
                 metricHost = None,
                 metricClock = None,
                 baseKey = "mongodb.server.collection",
                 **kwargs
                 ):
        self.server = server
    ...
    def update(self):
        for i, collection in self.db.printCollectionStats():
            collectionName = str(collection.ns).replace(self.dbName + '.', '')
            for k, v in self._flatten(d = collection).items():
                metric = Metric(
                                key = "%s.%s[%s,%s]" % (
                                                   self.baseKey,
                                                   k,
